...
def _close():
    ...
    print(f"_close:: {close_btn}")
    if exists(close_btn):
        moveAndClick(close_btn)
        return print('Found a close btn')
    openChest()

def _get_artifact():
    artifacts_list = [
        ['./img/utils/artifact.png', 300, 200, 1350, 830, 0.8, 1],
        ['./img/utils/artifact_2.png', 150, 50, 900, 450, 0.8, 1]
    ]
    print('open: _get_artifact')
    with concurrent.futures.ThreadPoolExecutor() as executor:
...
def _close_all_the_windows():
    ...
    while(not exists(_get_artifact())):
        if ((time.time() - start) > 100): # try to close the windows for 100s
            return _restart() 
        _zoom_out()
        _close()
...
def _check_if_app_started():
    ...
    while((time.time() - start) < time_limit):
        if exists(getImagePositionRegion(C.APP_START_STATIC, 0, 380, 120, 530, .8, 1)):
            return True
        delay(1)
    _restart()
    return _check_if_app_started()
